...
namespace Pacman_2
{
    public class Player
    {
        ...
        public Player(int tileX, int tileY, Tile[,] tileArray)
        {
            ...
            rectsDown[0] = new Rectangle(142, 239, 39, 39);
            rectsDown[1] = new Rectangle(191, 239, 39, 39);
            rectsDown[2] = new Rectangle(240, 239, 39, 39);
           

            rectsUp[0] = new Rectangle(143, 285, 39, 39);
            rectsUp[2] = new Rectangle(190, 285, 39, 39);
            rectsUp[1] = new Rectangle(238, 285, 39, 39);
           

            rectsLeft[0] = new Rectangle(0, 285, 39, 39);
            rectsLeft[1] = new Rectangle(52, 285, 39, 39);
            rectsLeft[2] = new Rectangle(98, 285,39, 39);
           
            rectsRight[0] = new Rectangle(1, 239, 39, 39);
            rectsRight[2] = new Rectangle(46, 239 ,39 , 39);
            rectsRight[1] = new Rectangle(92, 239,39 , 39);
        ...
        {
            int teleportResult = 0; // Variable to store teleport result

            // Call checkForTeleportPos only once and store the result
            int teleportCheck = checkForTeleport(tileArray);

            if (teleportCheck == 1)
            {
                if (direction == Dir.Left)
                    teleport(new Vector2(Game1.windowWidth + 30, position.Y), new Vector2(Controller.numberOfTilesX - 1, 17));
                teleportResult = 1;
            }
            else if (teleportCheck == 2)
            {
                if (direction == Dir.Right)
                    teleport(new Vector2(-30, position.Y), new Vector2(0, 17));
                teleportResult = 2;
            }
            else if (teleportCheck == 3)
            {
                if (direction == Dir.Left)
                    teleport(new Vector2(Game1.windowWidth + 30, position.Y), new Vector2(Controller.numberOfTilesX - 1, 8));
                teleportResult = 3;
            }
            else if (teleportCheck == 4)
            {
                if (direction == Dir.Right)
                    teleport(new Vector2(-30, position.Y), new Vector2(0, 8));
                teleportResult = 4;
            }

            return teleportResult; // Return the result of teleportation
        }

        // Additional method to check for teleportation condition
        private int checkForTeleport(Tile[,] tileArray)
        {
            if (new int[2] { (int)currentTile.X, (int)currentTile.Y }.SequenceEqual(new int[2] { 0, 17 }))
            {
                if (position.X < -30)
                {
                    return 1;
            ...
            else if (new int[2] { (int)currentTile.X, (int)currentTile.Y }.SequenceEqual(new int[2] { Controller.numberOfTilesX - 1, 17 }))
            {
            ...
            if (new int[2] { (int)currentTile.X, (int)currentTile.Y }.SequenceEqual(new int[2] { 0, 8 }))
            {
                if (position.X < -30)
                {
                    return 3;
            ...
            else if (new int[2] { (int)currentTile.X, (int)currentTile.Y }.SequenceEqual(new int[2] { Controller.numberOfTilesX - 1, 8 }))
            {
                if (position.X > tileArray[(int)currentTile.X, (int)currentTile.Y].Position.X + 30)
                {
                    return 4;
            ...
            return 0; // Default case when no teleport condition is met
        }
